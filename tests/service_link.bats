...
@test "($PLUGIN_COMMAND_PREFIX:link) exports SOLR_URL to app" {
  ...
  assert_contains "$url" "http://dokku-solr-ls:8983/solr/ls"
  assert_success
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" ls my-app
}

@test "($PLUGIN_COMMAND_PREFIX:link) generates an alternate config url when SOLR_URL already in use" {
  dokku config:set my-app SOLR_URL=http://host:8983
  dokku "$PLUGIN_COMMAND_PREFIX:link" ls my-app
...
@test "($PLUGIN_COMMAND_PREFIX:link) links to app with docker-options" {
  ...
  assert_contains "${lines[*]}" "--link dokku.solr.ls:dokku-solr-ls"
  assert_success
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" ls my-app
}

@test "($PLUGIN_COMMAND_PREFIX:link) uses apps SOLR_DATABASE_SCHEME variable" {
  dokku config:set my-app SOLR_DATABASE_SCHEME=solr2
  dokku "$PLUGIN_COMMAND_PREFIX:link" ls my-app
  url=$(dokku config:get my-app SOLR_URL)
  assert_contains "$url" "solr2://dokku-solr-ls:8983/solr/ls"
  assert_success
  dokku "$PLUGIN_COMMAND_PREFIX:unlink" ls my-app
}
